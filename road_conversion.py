...
class RoadHandler(osmium.SimpleHandler):
    ...
    def way(self, w):
        if 'highway' in w.tags:
            for i in tqdm(range(1, len(w.nodes)), desc="Processing Roads"):
...
def bresenham(x0, y0, x1, y1, arr):
    ...
    while x0 != x1 or y0 != y1:
        ...
        if 0 <= y0 < arr.shape[0] and 0 <= x0 < arr.shape[1]:
            arr[y0, x0] = 110  # mark the point as visited
        e2 = 2 * err
        if e2 > -dy:
            err -= dy
            x0 += sx
        if e2 < dx:
            err += dx
            y0 += sy
    # check if the final indices are within the bounds of the array
    if 0 <= y0 < arr.shape[0] and 0 <= x0 < arr.shape[1]:
        arr[y0, x0] = 110  # mark the final point as visited
...
if __name__ == '__main__':
    ...
    # osm_file_path = "/Users/aakash/Downloads/mandi.osm.pbf"
